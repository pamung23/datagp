...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                ...
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('lkkhusus.create', ['triwulan' => $triwulan]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('lkkhusus.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('lkkhusus.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
                    ...
                    <div class="float-left mr-2">
                        <a href="{{ route('lkkhusus.petaall') }}" class="btn btn-outline-primary btn-sm">
                            <div>
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>Alamat Lembaga Konservasi</th>
                        <th>Provinsi</th>
                        <th class="text-center" colspan="3">Lokasi Geografis (Desimal degree)</th>
                        <th>Luas Areal (Ha)</th>
                        <th class="text-center" colspan="3">Izin Lembaga Konservasi</th>
                        <th class="text-center" colspan="1">Koleksi pada Lembaga Konservasi</th>
                        <th class="text-center" colspan="4">Jumlah Koleksi (individu)</th>
                        <th>Penambah Data</th>
                        @if($triwulan !== 'all')
                        <th>Aksi</th>
                        @endif
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('lkkhusus.peta', ['triwulan' => $triwulan, 'id' => $item->id, 'latitude' => $item->latitude, 'longitude' => $item->longitude]) }}"
                        ...
                        <td class="text-center">{{ $item->luas_areal_hektar }}</td>
                        <td class="text-center">{{ $item->nomor }}</td>
                        <td class="text-center">{{ $item->tanggal }}</td>
                        <td class="text-center">{{ $item->masa_berlaku_tahun }}</td>
                        <td class="text-center">{{ $item->nama_ilmiah }}</td>
                        <td class="text-center">{{ $item->jantan }}</td>
                        <td class="text-center">{{ $item->betina }}</td>
                        <td class="text-center">{{ $item->belum_diketahui }}</td>
                        <td class="text-center">{{ $item->jumlah }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td>{{ $item->keterangan }}</td>
                        @if($triwulan !== 'all')
                        <td>
                            <a href="{{ route('lkkhusus.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form action="{{ route('lkkhusus.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('lkkhusus.index') }}";
