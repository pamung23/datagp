...
@section('title', 'Perencanaan Pengelolaan Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Perencanaan Pengelolaan Kawasan Konservasi</h4>
                </div>
                @if($semester !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('perencanaanpkk.create', ['semester' => $semester]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('perencanaanpkk.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2 mt-2">
                        @if ($year)
                        <a href="{{ route('perencanaanpkk.export', ['semester' => $semester, 'year' => $year]) }}"
                            class="btn btn-outline-success btn-sm">Export to Excel</a>
                        @else
                        <button id="excel-button" class="btn btn-outline-success btn-sm" disabled>Export to
                            Excel</button>
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" colspan="4">Rencana Pengelola Jangka Panjang</th>
                        <th class="text-center" colspan="4">Rencana Pengelolaan Jangka Pendek</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $item->jpan_nomor }}</td>
                        <td class="text-center">{{ $item->jpan_tanggal }}</td>
                        <td class="text-center">{{ $item->jpan_mulai }}</td>
                        <td class="text-center">{{ $item->jpan_akhir }}</td>
                        <td class="text-center">{{ $item->jpen_nomor }}</td>
                        <td class="text-center">{{ $item->jpen_tanggal }}</td>
                        <td class="text-center">{{ $item->jpen_periode }}</td>
                        <td class="text-center">{{ $item->jpen_luas }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td class="text-center">{{ $item->keterangan }}</td>
                        @if($semester !== 'all')
                        <td>
                            <a href="{{ route('perencanaanpkk.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('perencanaanpkk.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    semesterSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('perencanaanpkk.index') }}";
