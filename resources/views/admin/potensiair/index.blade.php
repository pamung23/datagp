...
@section('title', 'Potensi Pemanfaatan Air di Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Potensi Pemanfaatan Air di Kawasan Konservasi</h4>
                </div>
                @if($triwulan !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('potensiair.create', ['triwulan' => $triwulan]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('potensiair.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('potensiair.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
                    ...
                    <div class="float-left mr-2">
                        <a href="{{ route('potensiair.petaall') }}" class="btn btn-outline-primary btn-sm">
                            <div>
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th colspan="3" class="text-center">Koordinat Sumber Air
                            (Decimal Degree)</th>
                        <th colspan="2" class="text-center">Potensi Pemanfaatan
                            (Air dan Energi Air)</th>
                        <th class="text-center" colspan="2">Penetapan Areal Pemanfaatan Air</th>
                        <th>Pengusahaan oleh Pihak III</th>
                        <th>Penambah Data</th>
                        <th>Keterangan</th>
                        @if($triwulan !== 'all')
                        <th>Aksi</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->nama_sumber_air }}</td>
                        <td>{{ $item->debit }}</td>
                        <td>{{ $item->latitude }}</td>
                        <td>{{ $item->longitude }}</td>
                        <td>
                            <a href="{{ route('potensiair.peta', ['triwulan' => $triwulan, 'id' => $item->id, 'latitude' => $item->latitude, 'longitude' => $item->longitude]) }}"
                        ...
                        <td>{{ $item->massa_air }}</td>
                        <td>{{ $item->energi_air }}</td>
                        <td>{{ $item->nomor }}</td>
                        <td>{{ $item->tanggal }}</td>
                        <td>{{ $item->pengusahaan_pihak_iii }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td>{{ $item->keterangan }}</td>
                        @if($triwulan !== 'all')
                        <td>
                            <a href="{{ route('potensiair.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('potensiair.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('potensiair.index') }}";
