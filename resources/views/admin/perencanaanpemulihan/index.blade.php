...
@section('title', 'Kemitraan Konservasi')
        ...
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Perencanaan Pemuliahn Ekosistem Kawasan Konservasi</h4>
                </div>
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('perencanaanpemulihan.create') }}" class="btn btn-outline-primary btn-sm">Tambah
                        Data</a>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('perencanaanpemulihan.index') }}" method="GET" id="year-form" class="mb-4 ml-auto">
                ...
                <a href="{{ route('perencanaanpemulihan.export', ['year' => $selectedYear]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th colspan="3" class="text-center">Konservasi</th>
                    ...
                    <tr>
                        <th></th>
                        <th></th>
                        <th class="text-center">Tahun Dokumen RPE</th>
                ...
                <tbody>
                    @foreach ($perencanaanPemulihans as $item)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $item->no_register_kawasan }}</td>
                        <td>{{ $item->tahun_dokumen_rpe }}</td>
                        <td>{{ $item->metode_pe }}</td>
                        <td>{{ $item->target_pe }}</td>
                        <td class="">{{ $item->keterangan }}</td>
                        <td class="text-center">
                            <a href=" {{ route('perencanaanpemulihan.edit', $item->id) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form action="{{ route('perencanaanpemulihan.destroy', $item->id) }}" method="POST"
                                class="d-inline">
                                @csrf
                                @method('DELETE')
...
<script>
    $(document).ready(function () {
        ...
        yearSelect.addEventListener('change', function () {
            ...
            const baseUrl = "{{ route('perencanaanpemulihan.index') }}";
