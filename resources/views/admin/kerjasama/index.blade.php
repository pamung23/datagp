...
@section('title', 'kerjasama Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>kerjasama Kawasan Konservasi</h4>
                </div>
                @if($semester !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('kerjasama.create', ['semester' => $semester]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('kerjasama.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2 mt-2">
                        @if ($year)
                        <a href="{{ route('kerjasama.export', ['semester' => $semester, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Tipe Kerjasama</th>
                        <th class="text-center">kerjasama Kawasan Konservasi</th>
                        <th class="text-center">kerjasama Kawasan Konservasi</th>
                        <th class="text-center">kerjasama Kawasan Konservasi</th>
                        <th class="text-center">kerjasama Kawasan Konservasi</th>
                        <th class="text-center">kerjasama Kawasan Konservasi</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $item->tipe_kerjasama }}</td>
                        <td class="text-center">{{ $item->mitra_kerjasama }}</td>
                        <td class="text-center">{{ $item->judul_kerjasama }}</td>
                        <td class="text-center">{{ $item->ruang_lingkup_kerjasama }}</td>
                        <td class="text-center">{{ $item->nomor_mou }}</td>
                        <td>{{ $item->tanggal_mou }}</td>
                        <td>{{ $item->tanggal_awal_berlaku }}</td>
                        <td>{{ $item->tanggal_akhir_berlaku }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td class="text-center">{{ $item->keterangan }}</td>
                        @if($semester !== 'all')
                        <td>
                            <a href="{{ route('kerjasama.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('kerjasama.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    $('#zero-config').DataTable({
        ...
        semesterSelect.addEventListener('change', function() {
            ...
            const baseUrl = "{{ route('kerjasama.index') }}";
