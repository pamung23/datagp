...
@section('title', ' Perizinan Pemanfaatan Jasa Lingkungan pada Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4> Perizinan Pemanfaatan Jasa Lingkungan pada Kawasan Konservasi</h4>
                </div>
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('jasling.create', ['triwulan' => $triwulan]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('jasling.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('jasling.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>Resort</th>
                        <th colspan="8" class="text-center">Izin Pemanfaatan Massa Air</th>
                        <th>Penambah Data</th>
                        <th>Keterangan</th>
                        @if($triwulan !== 'all')
                        <th>Aksi</th>
                    ...
                    <tr>
                        ...
                        <th>IPA</th>
                        <th>IPEA</th>
                        <th>IPJLPB (Eksplorasi)</th>
                        <th>IPJLPB (Eksplorasi dan Pemanfaatan)</th>
                        <th></th>
                        <th></th>
                        @if($triwulan !== 'all')
                        <th></th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->ipa }}</td>
                        <td>{{ $item->ipea }}</td>
                        <td>{{ $item->ipjlpb_eksplorasi }}</td>
                        <td>{{ $item->ipjlpb_eksplorasi_pemanfaatan }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td>{{ $item->keterangan }}</td>
                        @if($triwulan !== 'all')
                        <td>
                            <a href="{{ route('jasling.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form action="{{ route('jasling.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('jasling.index') }}";
