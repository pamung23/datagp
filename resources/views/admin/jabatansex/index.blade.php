...
@section('title', 'Sebaran PNS/CPNS Menurut Jabatan dan Jenis Kelamin')
        ...
        <div class="widget-header">
            <div class="row py-2 m-auto">
                ...
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('jabatansex.create', ['semester' => $semester]) }}"
                        class="btn btn-outline-primary btn-sm">Tambah Data</a>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('jabatansex.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('jabatansex.export', ['semester' => $semester, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" colspan="24">Jenis Jabatan</th>
                    ...
                    <tr>
                        <th></th>
                        <th></th>
                        <th class="text-center" colspan="20">Struktural</th>
                        <th class="text-center" colspan="2">Fungsional Umum</th>
                        <th class="text-center" colspan="2">fungsional Tertentu</th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                    ...
                    <tr>
                        <th></th>
                        <th></th>
                        <th class="text-center" colspan="2">I-A</th>
                        <th class="text-center" colspan="2">I-B</th>
                        <th class="text-center" colspan="2">II-A</th>
                        <th class="text-center" colspan="2">II-B</th>
                        <th class="text-center" colspan="2">III-A</th>
                        <th class="text-center" colspan="2">III-B</th>
                        <th class="text-center" colspan="2">III-C</th>
                        <th class="text-center" colspan="2">III-D</th>
                        <th class="text-center" colspan="2">IV-A</th>
                        <th class="text-center" colspan="2">IV-B</th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                ...
                <tbody>
                    @foreach ($jabatan_sex as $item)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td class="text-center">{{ $item->satker_id }}</td>
                        <td>{{ $item->laki_ia }}</td>
                        <td>{{ $item->perempuan_ia }}</td>
                        <td>{{ $item->laki_ib }}</td>
                        <td>{{ $item->perempuan_ib }}</td>
                        <td>{{ $item->laki_iia }}</td>
                        <td>{{ $item->perempuan_iia }}</td>
                        <td>{{ $item->laki_iib }}</td>
                        <td>{{ $item->perempuan_iib }}</td>
                        <td>{{ $item->laki_iiia }}</td>
                        <td>{{ $item->perempuan_iiia }}</td>
                        <td>{{ $item->laki_iiib }}</td>
                        <td>{{ $item->perempuan_iiib }}</td>
                        <td>{{ $item->laki_iiic }}</td>
                        <td>{{ $item->perempuan_iiic }}</td>
                        <td>{{ $item->laki_iiid }}</td>
                        <td>{{ $item->perempuan_iiid }}</td>
                        <td>{{ $item->laki_iva }}</td>
                        <td>{{ $item->perempuan_iva }}</td>
                        <td>{{ $item->laki_ivb }}</td>
                        <td>{{ $item->perempuan_ivb }}</td>
                        <td>{{ $item->laki_umum }}</td>
                        <td>{{ $item->perempuan_umum }}</td>
                        <td>{{ $item->laki_tertentu }}</td>
                        <td>{{ $item->perempuan_tertentu }}</td>
                        <td>{{ $item->laki_jumlah }}</td>
                        <td>{{ $item->perempuan_jumlah }}</td>
                        <td>{{ $item->total }}</td>
                        <td>{{ $item->keterangan }}</td>
                        <td>
                            <a href="{{ route('jabatansex.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('jabatansex.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    // Ambil elemen select semester
        ...
        semesterSelect.addEventListener('change', function() {
            ...
            const baseUrl = "{{ route('jabatansex.index') }}";
