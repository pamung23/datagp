...
@section('title', 'Pemanfaatan Massa Air di Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
        ...
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Pemanfaatan Massa Air di Kawasan Konservasi</h4>
                </div>
                @if($triwulan !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('pemanfaatanair.create', ['triwulan' => $triwulan]) }}"
    ...
    <div class="widget-content widget-content-area br-6">
        <form action="{{ route('pemanfaatanair.index') }}" method="GET" class="mb-4 mt-3 ml-4">
            <div class="form-group d-flex">
                ...
                <div class="ml-auto mr-2">
                    @if ($year)
                    <a href="{{ route('pemanfaatanair.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
        ...
        <table id="zero-config" class="table table-striped" style="width:100%">
            <thead>
                <tr>
                    ...
                    <th colspan="3" class="text-center">Izin Pemanfaatan Massa Air</th>
                    <th colspan="2" class="text-center">Pemegang Izin</th>
                    <th colspan="2" class="text-center">Pemanfaatan Air</th>
                    <th>Debit yang dimanfaatkan (M3/Detik)</th>
                ...
                <tr>
                    ...
                    <th>Tanggal</th>
                    <th>Nama</th>
                    <th>Alamat Desa (ID Desa)</th>
                    <th>Jumlah yang dilayani (KK)</th>
                    <th>Lokasi Desa (ID Desa)</th>
                    <th></th>
                    <th></th>
                    <th></th>
                    <th></th>
            ...
            <tbody>
                @foreach ($pemanfaatanair as $item)
                <tr>
                    ...
                    <td>{{ $item->nama }}</td>
                    <td>
                        @if ($item->desa) Kabupaten
                        {{ $item->desa->kecamatan->kabupaten->nama }} / Kecamatan
                        {{ $item->desa->kecamatan->nama }} / Desa
                        {{ $item->desa->nama }}
                        @endif
                    </td>
                    <td>{{ $item->jumlah_dilayani_kk }}</td>
                    <td>
                        @if ($item->desa) Kabupaten
                        {{ $item->desa->kecamatan->kabupaten->nama }} / Kecamatan
                        {{ $item->desa->kecamatan->nama }} / Desa
                    ...
                    <td>{{ $item->nilai_investasi }}</td>
                    <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                    <td>{{ $item->keterangan }}</td>
                    @if($triwulan !== 'all')
                    <td>
                        <a href="{{ route('pemanfaatanair.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                            class="btn btn-outline-warning btn-sm">Edit</a>
                        <form
                            action="{{ route('pemanfaatanair.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('pemanfaatanair.index') }}";
