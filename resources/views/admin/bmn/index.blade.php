...
@section('title', 'Rincian Barang Milik Negara (Gabungan Intrakomptabel dan Ekstrakomptabel)')
        ...
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    ...
                    <h4>Rincian Barang Milik Negara (Gabungan Intrakomptabel dan Ekstrakomptabel)</h4>
                </div>
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('bmn.create', ['semester' => $semester]) }}"
                        class="btn btn-outline-primary btn-sm">Tambah Data</a>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('bmn.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('bmn.export', ['semester' => $semester, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" colspan="2">Kelompok Barang</th>
                        <th class="text-center">Satuan Barang</th>
                        <th class="text-center" colspan="2">Kondisi Sebelumnya</th>
                        <th class="text-center" colspan="4">Mutasi</th>
                        <th class="text-center" colspan="2">Saldo Barang</th>
                    ...
                    <tr>
                        ...
                        <th></th>
                        <th></th>
                        <th class="text-center" colspan="2">Bertambah</th>
                        <th class="text-center" colspan="2">Berkurang</th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->uraian }}</td>
                        <td>{{ $item->satuan }}</td>
                        <td>{{ $item->kuantitas1 }}</td>
                        <td>{{ $item->nilai1 }}</td>
                        <td>{{ $item->kuantitas2 }}</td>
                        <td>{{ $item->nilai2 }}</td>
                        <td>{{ $item->kuantitas3 }}</td>
                        <td>{{ $item->nilai3 }}</td>
                        <td>{{ $item->kuantitas4 }}</td>
                        <td>{{ $item->nilai4 }}</td>
                        <td>{{ $item->keterangan }}</td>
                        <td>
                            <a href="{{ route('bmn.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form action="{{ route('bmn.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
