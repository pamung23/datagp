...
@section('title', 'Pengusahaan Pemanfaatan Jasa Lingkungan Wisata Alam')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Pengusahaan Pemanfaatan Jasa Lingkungan Wisata Alam</h4>
                </div>
                @if($triwulan !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('iupjswa.create', ['triwulan' => $triwulan]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('iupjswa.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('iupjswa.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>Resort</th>
                        <th>Nama Zona/Blok Pemanfaatan</th>
                        <th>Luas Zona/Blok Pemanfaatan (Ha)</th>
                        <th colspan="3" class="text-center">IUPSWA</th>
                        <th colspan="3" class="text-center">IUPJWA</th>
                        <th>Penambah Data</th>
                        <th>Keterangan</th>
                        @if($triwulan !== 'all')
                        <th>Aksi</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->iupswa_nama_perusahaan }}</td>
                        <td>{{ $item->iupswa_tahun_penerbitan }}</td>
                        <td>{{ $item->iupswa_luas_area }}</td>
                        <td>{{ $item->iupjwa_nama_pemegang_izin }}</td>
                        <td>{{ $item->iupjwa_tahun_penerbitan_izin }}</td>
                        <td>{{ $item->iupjwa_jenis_jasa }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td>{{ $item->keterangan }}</td>
                        @if($triwulan !== 'all')
                        <td>
                            <a href="{{ route('iupjswa.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form action="{{ route('iupjswa.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('iupjswa.index') }}";
