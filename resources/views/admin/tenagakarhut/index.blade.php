...
@section('title', 'Tenaga Pengendalian Kebakaran Hutan')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Tenaga Pengendalian Kebakaran Hutan</h4>
                </div>
                @if($triwulan !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('tenagakarhut.create', ['triwulan' => $triwulan]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('tenagakarhut.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('tenagakarhut.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th>Resort</th>
                        <th colspan="4" class="text-center">Tenaga Pengendalian Kebakaran Hutan</th>
                        <th>Penambah Data</th>
                        <th>Keterangan</th>
                        @if($triwulan !== 'all')
                        <th>Aksi</th>
                    ...
                    <tr>
                        ...
                        <th></th>
                        <th></th>
                        <th>Manggala Agni PNS (Orang)</th>
                        <th>Manggala Agni Non PNS (Orang)</th>
                        <th>Jumlah Regu</th>
                        <th>MPA (Orang)</th>
                        <th></th>
                        <th></th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $item->manggala_agni_pns }}</td>
                        <td class="text-center">{{ $item->manggala_agni_nonpns }}</td>
                        <td class="text-center">{{ $item->jumlah_regu }}</td>
                        <td class="text-center">{{ $item->mpa }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td>{{ $item->keterangan }}</td>
                        @if($triwulan !== 'all')
                        <td>
                            <a href="{{ route('tenagakarhut.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('tenagakarhut.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('tenagakarhut.index') }}";
