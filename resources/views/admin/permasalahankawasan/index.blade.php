...
@section('title', 'Permasalahan Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Permasalahan Kawasan Konservasi</h4>
                </div>
                @if($triwulan !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('permasalahankawasan.create', ['triwulan' => $triwulan]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('permasalahankawasan.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2">
                        @if ($year)
                        <a href="{{ route('permasalahankawasan.export', ['triwulan' => $triwulan, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>No. Register Kawasan</th>
                        <th>Resort</th>
                        <th>Jenis Permasalahan</th>
                        <th>Uraian Permasalahan</th>
                        <th>Progres Penyelesaian</th>
                        <th>Penambah Data</th>
                        <th>Keterangan</th>
                        @if($triwulan !== 'all')
                        <th>Aksi</th>
                ...
                <tbody>
                    @foreach ($permasalahanKawasan as $item)
                    <tr>
                        ...
                        <td>{{ $item->progres_penyelesaian }}</td>
                        <td>{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td>{{ $item->keterangan }}</td>
                        @if($triwulan !== 'all')
                        <td>
                            <a href="{{ route('permasalahankawasan.edit', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('permasalahankawasan.destroy', ['triwulan' => $triwulan, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    triwulanSelect.addEventListener('change', function () {
        ...
        const baseUrl = "{{ route('permasalahankawasan.index') }}";
