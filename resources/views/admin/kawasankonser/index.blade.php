...
@section('title', 'Kawasan Konservasi yang Mendapat Penetapan Status Internasional sebagai Cagar Biosfer')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Kawasan Konservasi yang Mendapat Penetapan Status Internasional sebagai Cagar Biosfer</h4>
                </div>
                @if($semester !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('kawasankonservasi.create', ['semester' => $semester]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('kawasankonservasi.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2 mt-2">
                        @if ($year)
                        <a href="{{ route('kawasankonservasi.export', ['semester' => $semester, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Nama Cagar Biosfer</th>
                        <th class="text-center">Tahun Penetapan</th>
                        <th class="text-center" colspan="3">Kawasan Konservasi Di Dalam Cagar Biosfer </th>
                    ...
                    <tr>
                        ...
                        <th></th>
                        <th></th>
                        <th class="text-center" colspan="1">Area Inti</th>
                        <th class="text-center" colspan="1">Zona Penyangga</th>
                        <th class="text-center" colspan="1">Area Transisi</th>
                        <th></th>
                        <th></th>
                        @if($semester !== 'all')
                        <th></th>
                ...
                <tbody>
                    @foreach ($kawasan_konservasi as $item)
                    <tr>
                        ...
                        <td class="text-center">{{ $item->nama_cagar_biosfer }}</td>
                        <td class="text-center">{{ $item->tahun_penetapan }}</td>
                        <td class="text-center">{{ $item->area_inti }}</td>
                        <td class="text-center">{{ $item->zona_penyangga }}</td>
                        <td class="text-center">{{ $item->area_transisi }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td class="text-center">{{ $item->keterangan }}</td>
                        @if($semester !== 'all')
                        <td class="text-center">
                            <a href="{{ route('kawasankonservasi.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('kawasankonservasi.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    $('#zero-config').DataTable({
        ...
        semesterSelect.addEventListener('change', function() {
            ...
            const baseUrl = "{{ route('kawasankonservasi.index') }}";
