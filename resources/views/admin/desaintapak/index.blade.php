        ...
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Desain Tapak Pemanfaatan</h4>
                </div>
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('desaintapak.create') }}" class="btn btn-outline-primary btn-sm">Tambah
                        Data</a>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('desaintapak.index') }}" method="GET" id="year-form" class="mb-4 ml-auto">
                ...
                <a href="{{ route('desaintapak.export', ['year' => $selectedYear]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Nama Zona/Blok Pemanfataan</th>
                        <th class="text-center">Luas Zona/Blok Pemanfaatan (Ha)</th>
                        <th colspan="2" class="text-center">Desain Tapak</th>
                    ...
                    <tr>
                        ...
                        <th></th>
                        <th></th>
                        <th>Ruang Publik (Ha)</th>
                        <th>Ruang Usaha (Ha)</th>
                        <th></th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($desainTapaks as $item)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $item->no_register_kawasan}}</td>
                        <td>{{ $item->nama_zona_blok_pemanfaatan}}</td>
                        <td>{{ $item->luas_zona_blok_pemanfaatan }}</td>
                        <td>{{ $item->ruang_publik }}</td>
                        <td>{{ $item->ruang_usaha }}</td>
                        <td>{{ $item->keterangan }}</td>
                        <td>
                            <a href="{{ route('desaintapak.edit', $item->id) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form action="{{ route('desaintapak.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
...
<script>
    $(document).ready(function () {
        ...
        yearSelect.addEventListener('change', function () {
            ...
            const baseUrl = "{{ route('desaintapak.index') }}";
