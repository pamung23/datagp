...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Sebaran Pejabat Fungsional Tertentu Menurut Fungsi dan Jenis Kelamin</h4>
                </div>
                @if($semester !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('fungsionalsex.create', ['semester' => $semester]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('fungsionalsex.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2 mt-2">
                        @if ($year)
                        <a href="{{ route('fungsionalsex.export', ['semester' => $semester, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center" colspan="18">Jenis Jabatan Fungsional Tertentu</th>
                    ...
                    <tr>
                        ...
                        <th class="text-center" colspan="2">Polhut</th>
                        <th class="text-center" colspan="2">Penyuluh</th>
                ...
                <tbody>
                    @foreach ($fungsional_sex as $item)
                    <tr>
                        ...
                        <td>{{ $item->laki_peh }}</td>
                        <td>{{ $item->perempuan_peh }}</td>
                        <td>{{ $item->laki_polhut }}</td>
                        <td>{{ $item->perempuan_polhut }}</td>
                        <td>{{ $item->laki_penyuluh }}</td>
                        <td>{{ $item->perempuan_penyuluh }}</td>
                        <td>{{ $item->laki_pranata }}</td>
                        <td>{{ $item->perempuan_pranata }}</td>
                        <td>{{ $item->laki_statistisi }}</td>
                        <td>{{ $item->perempuan_statistisi }}</td>
                        <td>{{ $item->laki_analis }}</td>
                        <td>{{ $item->perempuan_analis }}</td>
                        <td>{{ $item->laki_arsiparis }}</td>
                        <td>{{ $item->perempuan_arsiparis }}</td>
                        <td>{{ $item->laki_perencana }}</td>
                        <td>{{ $item->perempuan_perencana }}</td>
                        <td>{{ $item->laki_pengadaan }}</td>
                        <td>{{ $item->perempuan_pengadaan }}</td>
                        ...
                        @if($semester !== 'all')
                        <td>
                            <a href="{{ route('fungsionalsex.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('fungsionalsex.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    $('#zero-config').DataTable({
        ...
        semesterSelect.addEventListener('change', function() {
            ...
            const baseUrl = "{{ route('fungsionalsex.index') }}";
