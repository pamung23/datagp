...
@section('title', 'Hasil Evaluasi Kesesuaian Fungsi Kawasan Konservasi')
...
<div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row py-2 m-auto">
                <div class="col-xl-6 col-md-6 col-sm-6 col-6">
                    <h4>Hasil Evaluasi Kesesuaian Fungsi Kawasan Konservasi</h4>
                </div>
                @if($semester !== 'all')
                <div class="col-xl-6 col-md-6 col-sm-6 col-6 text-right m-auto">
                    <a href="{{ route('hasilevaluasi.create', ['semester' => $semester]) }}"
        ...
        <div class="widget-content widget-content-area br-6">
            <form action="{{ route('hasilevaluasi.index') }}" method="GET" class="mb-4 mt-3 ml-4">
                <div class="form-group d-flex">
                    ...
                    <div class="ml-auto mr-2 mt-2">
                        @if ($year)
                        <a href="{{ route('hasilevaluasi.export', ['semester' => $semester, 'year' => $year]) }}"
            ...
            <table id="zero-config" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Tanggal EKF</th>
                        <th class="text-center">Rekomendasi EKF</th>
                        <th class="text-center">Tindak Lanjut EKF</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $item->tindak }}</td>
                        <td class="text-center">{{ $item->user ? $item->user->nama_lengkap : 'Unknown User' }}</td>
                        <td class="text-center">{{ $item->keterangan }}</td>
                        @if($semester !== 'all')
                        <td>
                            <a href="{{ route('hasilevaluasi.edit', ['semester' => $semester, 'id' => $item->id]) }}"
                                class="btn btn-outline-warning btn-sm">Edit</a>
                            <form
                                action="{{ route('hasilevaluasi.destroy', ['semester' => $semester, 'id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    // Ambil elemen select semester
        ...
        semesterSelect.addEventListener('change', function() {
            ...
            const baseUrl = "{{ route('hasilevaluasi.index') }}";
