...
class PegawaiPendidikanExport implements FromCollection, WithHeadings, WithStyles, WithMapping
{
    use Exportable;
    ...
    public function headings(): array
    {
        return [
            [
                'Tabel : Sebaran PNS/CPNS Menurut Tingkat Pendidikan dan Jenis Kelamin',
    ...
    public function map($row): array
    {
        return [
            ...
            $row->nomor_urut,
            $row->satker_id,
            $row->laki_doktor, // Sesuaikan dengan nama kolom yang sesuai dengan data yang akan diekspor
            $row->perempuan_doktor, // Sesuaikan dengan nama kolom yang sesuai dengan data yang akan diekspor
            $row->laki_master, // Sesuaikan dengan nama kolom yang sesuai dengan data yang akan diekspor
            $row->perempuan_master,
            $row->laki_sarjana,
            $row->perempuan_sarjana,
            $row->laki_sarjana_muda,
            $row->perempuan_sarjana_muda,
            $row->laki_slta,
            $row->perempuan_slta,
            $row->laki_sltp,
            $row->perempuan_sltp,
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        foreach ($styles as $row => $style) {
            ...
            $sheet->getStyle('B' . $row . ':U' . $row)->applyFromArray($style);
        }
        ...
        $sheet->getStyle('B7:U' . $highestRow)->getAlignment()->setHorizontal('center');
        $sheet->getStyle('B7:U' . $highestRow)->getAlignment()->setVertical('center');
